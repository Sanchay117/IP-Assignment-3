...
class Gaussians:
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        if self.is_isotropic:
            ...
            ### YOUR CODE HERE ###
             # scales: (N, 1) => cov = s^2 * I
            s_squared = scales.squeeze(-1) ** 2  # (N,)
            cov_3D = torch.eye(3, device=scales.device).unsqueeze(0) * s_squared[:, None, None]  # (N, 3, 3)
        ...
        # HINT: You can use a function from pytorch3d to convert quaternions to rotation matrices.
            ...
            cov_3D = R @ S_st @ R.transpose(1, 2)  # (N, 3, 3), full formula: R S S^T R^T
    ...
    def compute_cov_2D(
        ...
        # A = W @ Σ_3D @ W^T
        A = W @ cov_3D @ W.transpose(1, 2)  # (N, 3, 3)
        cov_2D = J @ A @ J.transpose(1, 2)  # (N, 2, 2)
